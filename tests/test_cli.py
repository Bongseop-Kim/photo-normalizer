...
def test_main_rejects_output_same_as_input(tmp_path, monkeypatch):
    ...
    input_dir.mkdir()
    monkeypatch.setattr("sys.argv", ["normalize.py", str(input_dir), "--output", str(input_dir)])
...
def test_main_rejects_missing_explicit_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sys.argv", ["normalize.py", str(input_dir), "--config", str(missing)])
